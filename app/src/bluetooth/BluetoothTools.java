...
public class BluetoothTools {
     ...
     * 蓝牙设备最小的收音机发射频率值
     */
    public final static int MIN_FREQUENCY = 875;


    /**
     * 蓝牙设备最大的收音机发射频率值
     */
    public final static int MAX_FREQUENCY = 1080;
    ...
    public BluetoothTools(BluetoothDevice dev) {
        if (dev == null) {
            throw new com.xhk.bluetooth.XHKException("The BluetoothDevice must not be null!");
        }
        this.dev = dev;
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    Log.e(TAG, dev.getName() + "[" + dev.getAddress() + "] connection success", e);
                    if (listenter != null) {
    ...
    private int getFMFrequencyOnce() {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            throw new com.xhk.bluetooth.XHKException("Please don't run it in Main Thread!");
        }
        try {
...
//            }
            ...
            in.read(resp);
            Log.d(TAG, "resp==>" + com.xhk.bluetooth.Utils.bytes2HexString(resp));
            byte b[] = new byte[4];
            System.arraycopy(resp, 12, b, 0, 4);
            return com.xhk.bluetooth.Utils.byte2int(b);
        } catch (Exception e) {
            e.printStackTrace();
            Log.e(TAG, "", e);
            return -1;
...
//        }
     ...
     * 设置硬件设备的收音机接收频率
     *
     * @param frequency 频率值，最大为1080，最小为875
     */
    private void setFMFrequencyOnce(int frequency) {
        ...
        byte[] freByte = com.xhk.bluetooth.Utils.int2byte(frequency);
        try {
            ...
            bytes[10] = freByte[2];
            bytes[11] = freByte[3];
            Log.d(TAG, "requ==>" + com.xhk.bluetooth.Utils.bytes2HexString(bytes));
            out.write(bytes);
    ...
    private int getSwitchState(byte commond) {
        ...
        try {
            ...
            Log.d(TAG, "req ==>" + com.xhk.bluetooth.Utils.bytes2HexString(bytes));
            out.write(bytes);
            int count = 0;
//            while (count == 0) {
            count = in.available();
